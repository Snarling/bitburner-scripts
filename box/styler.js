import {doc,createBox,sidebar} from "/box/box.js";
export let main=async ns=>{
  ...
  let box=createBox('styler', `<button>Load from File</button><button>Load from Page</button><button>Save to File</button><button>Save to Page</button><div class=resizer style="min-width:511px;height:300px"><textarea spellcheck=false></textarea></div><button>Load Theme from Game</button><button>Save Theme to Game</button><button>Minify</button><button>Beautify</button>`,"&#xeb5c"),
  ...
  box.querySelectorAll("button").forEach((button,i)=>button.addEventListener('click',events[i]));
  events[0]();

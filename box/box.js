let css=`body{--prilt:#FFF;--pri:#F8F8F2;--pridk:#FAFAEB;--successlt:#ADE146;--success:#A6E22E;--successdk:#98E104;--errlt:#FF69A0;--err:#F92672;--errdk:#D10F56;--seclt:#AAA;--sec:#888;--secdk:#666;--warnlt:#E1D992;--warn:#E6DB74;--warndk:#EDDD54;--infolt:#92E1F1;--info:#66D9EF;--infodk:#31CDED;--welllt:#444;--well:#222;--white:#fff;--black:#000;--hp:#F92672;--money:#E6DB74;--hack:#A6E22E;--combat:#75715E;--cha:#AE81FF;--int:#66D9EF;--rep:#E69F66;--disabled:#66cfbc;--bgpri:#272822;--bgsec:#1B1C18;--button:#333;--ff:"JetBrains Mono"}body{overflow:hidden;display:flex}#root{flex:1 1 calc(100vw - 248px);overflow:scroll}.sb{font:14px var(--ff);color:var(--pri);background:var(--bgsec);box-sizing:border-box;overflow-y:scroll;overflow-x:hidden;width:248px;min-height:100%;border-left:1px solid var(--welllt);transition:width 250ms}.sb *:not(table *){color:inherit}.sb *{vertical-align:middle;margin:0;font:inherit}.sb.c{width:45px}.sb .item{transition:color 200ms;border-top:1px solid var(--welllt);background:var(--bgsec);line-height:1.5}.sb .box{position:fixed;border:1px solid var(--welllt);width:min-content}.sb .head{direction:rtl;display:flex;white-space:pre;cursor:pointer;user-select:none;padding:8px 0px}.sb .item.c{color:var(--sec)}.box>.head{direction:ltr;background:var(--bgpri);padding:2px;cursor:move;border-bottom:1px solid var(--welllt)}.box>.body{padding:2px}.sb .title{margin:0px auto}.box .title{margin-left:10px;margin-right:auto}.sb .close:not(.box *){display:none}.sb>:not(.box)>.body{resize:vertical;overflow:hidden;transition:height 250ms;font-size:12px}.sb :is(.c .body):not(.box *){height:0px !important}.box.c>.body{display:none}.box.prompt{box-shadow:0 0 0 10000px #0007;min-width:400px}.sb .contextMenu{opacity:0.95;border-radius:5px;background:var(--bgpri)}.sb .contextMenu .head{display:none}.sb .icon{padding:0px 10px;cursor:pointer;font:25px "codicon";line-height:0.9}.box .icon{padding:0}.sb .icon.lr::after{content:""}.sb.c .icon.lr::after{content:""}.sb .icon.ud::after{content:""}.sb .c .icon.ud::after{content:""}.sb :is(input,select,button,textarea){outline:none;border:none}.sb :is(textarea,.log){width:100%;white-space:pre-wrap;font-size:12px;background:none;padding:0px;height:100%;overflow-y:scroll}.sb :is(input,select){padding:3px;background:var(--well);border-bottom:1px solid var(--prilt);transition:border-bottom 250ms}.sb input:hover{border-bottom:1px solid var(--black)}.sb input:focus{border-bottom:1px solid var(--prilt)}.sb :is(button,input[type=checkbox]){background:var(--button);transition:background 250ms;border:1px solid var(--well)}.sb :is(button,input[type=checkbox]):hover{background:var(--bgsec)}.sb :is(button,input[type=checkbox]):focus, .sb select{border:1px solid var(--secdk)}.sb button{padding:6px 8px;user-select:none}.sb .ts{color:var(--infolt)}.sb input[type=checkbox]{appearance:none;display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px}.sb input[type=checkbox]:checked::after{font:22px codicon;content:""}.sb .g2{display:grid;grid:auto-flow minmax(max-content, 25px) / repeat(2, minmax(max-content, auto));gap:6px;margin:3px;place-items:center start}.sb .g2>*:nth-child(odd){justify-self:end}.sb .full{text-align:center;width:100%}.sb .resizer{width:100%;min-width:99%;resize:both;overflow:hidden}`;
...
export let createSidebarItem=(title,content,icon="",...classes)=>{
  let item=sidebar.appendChild(elemFromHTML(`<div class="${[...classes,"item"].join(" ")}"><div class="head"><a class="icon">${icon}</a><span class=title>${title}</span><a class="icon ud"></a><a class="icon close">&#xeab8</a></div><div class="body">${content}</div></div>`));
...
let contextMenu=(item,x,y)=>{
  ...
  let options = Object.entries(item.contextItems).filter(([name,entry])=>entry.cFn());
	let box=createBox("",`<div class=full>${item.querySelector(".title").innerText}.context</div><div class=g2>${options.map(([name,entry])=>`<button>${name}</button>`).join("")}</div>`,"", "contextMenu");
	box.querySelector("button").focus();

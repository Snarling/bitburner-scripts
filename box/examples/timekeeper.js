...
export async function main(ns) {
  ...
  let item=createSidebarItem("timekeeper", `${style}<div class="f clock"><div>${"<div></div>".repeat(5)}</div>${`<div>${"<div></div>".repeat(6)}</div>`.repeat(2)}</div>`, "\ueb7c");
  ...
  item.contextItems.push(["Toggle Numbers",()=>{

...
export let main=ns=>{
  let item=createSidebarItem("overview</span><span class=icon name=save>\ueb4b</span>",`<style>.react-draggable:first-of-type{display:none}.sb .overview{padding:10px;font-size:14px}.sb .overview table{display:table}</style><div class=overview>${doc.querySelector(".react-draggable:first-of-type table").outerHTML}</div>`,"\ueb03");

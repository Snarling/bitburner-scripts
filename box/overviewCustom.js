...
export let main=ns=>{
  let overview=createSidebarItem("overview",`<style>.react-draggable:first-of-type{display:none}.overview .icon{position:relative}.overview{--str:var(--combat);--def:var(--combat);--dex:var(--combat);--agi:var(--combat);row-gap:2px !important;font-size:14px}.overview>.icons{border-top:1px solid var(--pri);width:100%;display:grid;grid:auto-flow 30px / repeat(auto-fit, minmax(30px, auto));place-items:center}.overview>.bar{height:3px;width:100%;margin-top:-10px;overflow:hidden;background:var(--black)}.overview>.bar>*{height:100%}${stats.map(stat=>`.overview .${stat}{color:var(--${stat})}.overview .bg-${stat}{background:var(--${stat})}`).join("")}</style>
